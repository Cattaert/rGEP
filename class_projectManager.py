...
def saveAsim(samplePt, obj_animatLabModel, fldrSimFiles,
             ix, indexLen=3, verbose=3, affiche=True):
    """
    ...
    for ptVar in samplePt:
        ...
        """
        print("\n\n{%s} = {%s} >> {%s}".format(ptVar, node.find(param).text,
                                               samplePt[ptVar]))
        """
    ...
    obj_animatLabModel.saveXML(fileName=os.path.join(fldrSimFiles, filename),
                               overwrite=True, affiche=affiche)
...
class ProjectManager(object):
    ...
    def make_asims(self, obj_simSet):
        ...
        results = pool.map(saveAsimWrapper,
                           [(pts, copy(self.aproj), self.simRunner.simFiles,
                             ix, countLength, verbose, False)
